...
# full grid -- halving will prune it down efficiently
PARAM_GRID = {
    "hidden_layer_sizes": [(64, 32), (32, 32), (64, 32, 16), (128, 64)],
...
def process(csv_path):
    ...
    for rnd in range(N_ROUNDS):
        ...
        base = MLPRegressor(**MLP_FIXED, random_state=seed)
        ...
        pred = search.best_estimator_.predict(Xte_s)
        rows.append({
            "dataset":     tag,
            "n_feats":     n_feats,
            "snr":         snr,
...
if __name__ == "__main__":
    ...
    pd.DataFrame(all_rows).to_csv("results_mlp_halving.csv", index=False)
    print(f"\ndone. {len(all_rows)} rows -> results_mlp_halving.csv")